...
// Request type masks/flags (see virtio-blk spec).
const (
	blkTypeMask    = 0xFF
	blkFlagBarrier = 1 << 31 // legacy barrier flag
	blkFlagFUA     = 1 << 30 // force unit access
)
...
func (h *blockHandler) GetFeatures() (features uint64) {
	...
	// Advertise FLUSH support - required for FUA (Force Unit Access) requests
	return blkFFlush
}
...
func (h *blockHandler) handle(q *virtq.Queue) error {
	for {
		...
		case blkTOut:
			...
			if err == nil && (flags&blkFlagFUA) != 0 {
				if syncer, ok := h.cfg.Storage.(interface{ Sync() error }); ok {
					err = syncer.Sync()
				}
			}

		case blkTFlush:
			// Flush/sync the backing storage
			// For FileStorage, call Sync() on the underlying file
			if syncer, ok := h.cfg.Storage.(interface{ Sync() error }); ok {
				err = syncer.Sync()
			}
			// If storage doesn't support Sync, that's OK - just succeed
			// The OS will handle flushing eventually

		default:
			status[0] = blkSUnsupp
		}

		// Barrier is legacy; best-effort sync when requested.
		if err == nil && (flags&blkFlagBarrier) != 0 {
			if syncer, ok := h.cfg.Storage.(interface{ Sync() error }); ok {
				err = syncer.Sync()
			}
